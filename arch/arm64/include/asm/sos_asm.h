...
#ifndef __ARM_SOS_ASM_H__
#define __ARM_SOS_ASM_H__
...
//debug macros
#define sos_err(fmt, ...) \
	pr_err("sos [%i]: " fmt, task_pid_nr(current), ## __VA_ARGS__)
#define sos_info(fmt, ...) \
	pr_info("sos [%i]: " fmt, task_pid_nr(current), ## __VA_ARGS__)
...
#define __KVM_HOST_SMCCC_FUNC___sos_enter			1
...
#define DECLARE_SOS_SYM_PER_CPU(type, sym)	\
	DECLARE_PER_CPU(type, sos_hyp_sym(sym))

#define DECLARE_KVM_HYP_PER_CPU(type, sym)	\
	DECLARE_PER_CPU(type, sym);	\
	DECLARE_SOS_SYM_PER_CPU(type, sym)
//
/*
...
#define per_cpu_ptr_nvhe_sym(sym, cpu)						\
	({									\
		...
		off = (unsigned long)&CHOOSE_SOS_HYP_SYM(sym) -			\
...
#define __kvm_hyp_init		CHOOSE_SOS_HYP_SYM(__kvm_hyp_init)
#define __kvm_hyp_vector	CHOOSE_SOS_HYP_SYM(__kvm_hyp_vector)

extern unsigned long kvm_arm_hyp_percpu_base[NR_CPUS];
DECLARE_SOS_HYP_SYM(__per_cpu_start);
DECLARE_SOS_HYP_SYM(__per_cpu_end);
