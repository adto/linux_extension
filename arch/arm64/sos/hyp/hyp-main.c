...
static void handle___sos_enter(struct kvm_cpu_context *host_ctxt)
{
	DECLARE_REG(struct kvm_vcpu *, vcpu, host_ctxt, 1);

	cpu_reg(host_ctxt, 1) =  __sos_enter(kern_hyp_va(vcpu));
}

static void handle___sos_exit(struct kvm_cpu_context *host_ctxt)
{
	DECLARE_REG(struct kvm_vcpu *, vcpu, host_ctxt, 1);

	cpu_reg(host_ctxt, 1) =  __sos_exit(kern_hyp_va(vcpu));
}
...
static const hcall_t host_hcall[] = {
	HANDLE_FUNC(__sos_enter),
	HANDLE_FUNC(__sos_exit),
...
void handle_trap(struct kvm_cpu_context *host_ctxt)
{
	...
	case ESR_ELx_EC_SVC64:
		handle_svc(host_ctxt);
		break;
	case ESR_ELx_EC_ERET:
		handle_eret(host_ctxt);
		break;
	default:
		BUG();
	}
